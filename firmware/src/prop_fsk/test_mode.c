...
static K_THREAD_STACK_DEFINE(test_tx_thread_stack, PFSK_TEST_TX_THREAD_STACK_SIZE);
static K_THREAD_STACK_DEFINE(test_rx_thread_stack, PFSK_TEST_RX_THREAD_STACK_SIZE);
...
static void pfsk_test_prepare_tx_packet(struct pfsk_packet *packet)
{
	...
	*packet = (struct pfsk_packet){
		.length = PFSK_PACKET_METADATA_LEN + test_payload_len,
	};
...
{
	test_payload_len = PFSK_PAYLOAD_MAX_LEN;
	k_event_clear(&test_events, PFSK_TEST_EVENT_RUNNING);
}
...
static void pfsk_test_start_traffic(size_t payload_len)
{
	...
	k_event_set(&test_events, PFSK_TEST_EVENT_RUNNING);
}
...
static void pfsk_test_tx_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (1) {
		while ((k_event_wait(&test_events, PFSK_TEST_EVENT_RUNNING, false, K_FOREVER) &
