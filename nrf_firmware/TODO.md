...
- Impact: `pgfsk_link_set_config()` callers such as `linktest off`, `set role`, or `set mode` can time out after 1 second because the link thread stops servicing `g_pgfsk_config_queue` until an RF packet arrives.
...
- Files: `src/pgfsk/link.c`, `src/app_control.c`
- Problem: both `pgfsk_link_set_config()` and `app_control_set()` queue request structs that contain pointers to stack-local completion state (`struct k_sem *done`, `bool *result`) and then wait with fixed timeouts.
