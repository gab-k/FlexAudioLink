...
void prop_gfsk_radio_hw_init(void)
{
	...
	nrf_radio_base0_set(NRF_RADIO, PROP_GFSK_RADIO_SYNC_WORD);
	nrf_radio_base1_set(NRF_RADIO, PROP_GFSK_RADIO_SYNC_WORD);
...
void prop_gfsk_radio_hw_start(void)
{
	...
	nrf_dppi_channels_enable(PROP_GFSK_DPPI,
...
void prop_gfsk_radio_hw_set_role(enum device_role role)
{
	nrf_radio_prefix0_set(NRF_RADIO,
			      PROP_GFSK_RADIO_ADDR_PREFIX_DONGLE |
				      ((uint32_t)PROP_GFSK_RADIO_ADDR_PREFIX_HEADSET << 8));
...
{
	/* Trigger a capture into an unused CC channel then read it. */
	nrf_timer_task_trigger(PROP_GFSK_TIMER, NRF_TIMER_TASK_CAPTURE3);
	return nrf_timer_cc_get(PROP_GFSK_TIMER, NRF_TIMER_CC_CHANNEL3);
}
...
void prop_gfsk_radio_hw_schedule_tx(uint32_t tx_tick, const struct prop_gfsk_packet *packet)
{
	...
	/*
	 * Copy the packet before arming the timer so the pre-TX ISR always
	 * sees a consistent g_tx_packet regardless of when it fires.
	 */
	...
	nrf_timer_cc_set(PROP_GFSK_TIMER,
			 	PROP_GFSK_TIMER_CC_PRE_TX,
			 	  tx_tick - PROP_GFSK_PRE_TX_OFFSET_US);
